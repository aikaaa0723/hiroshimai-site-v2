(function () {
  ...
  function buildParticles() {
    ...
    var step = narrow ? 8 : 6;
    var xOffset = narrow ? 0.06 : 0.50;
    var xScale = narrow ? 1.20 : 1.68;
    var yScale = narrow ? 1.50 : 1.82;

    for (var y = 0; y < side; y += step) {
      for (var x = 0; x < side; x += step) {
        var alpha = pixels[(y * side + x) * 4 + 3];
        if (alpha < 42 || Math.random() > 0.84) continue;
  ...
  function render(now) {
    var time = now * 0.001;
    var idleX = 0.50 + Math.cos(time * 0.31) * 0.30;
